...
使用方法：
    python plc_audit.py <plc_xml_file> <excel_file1> [excel_file2 ...] [output_dir]

参数：
    plc_xml_file: PLC源码XML文件路径（必需）
    excel_files: 一个或多个Excel IO数据库文件路径（必需）
    output_dir: 输出目录（可选，通过-o或--output指定，默认为当前目录）

示例：
    python plc_audit.py code.xml io_table.xlsx
...
def parse_plc_variables(plc_files):
    """
    解析PLC文件（支持XML或JSON Lines格式），提取变量
    支持单个文件路径或文件列表
    返回: list of dict {name, type, line, context, source_file}
    """
    # 统一为列表
...
def parse_excel_io_db(excel_files, output_dir=None):
    ...
    分别处理IO表和IO数据库，避免数据覆盖
    返回: dict {io地址: {变量名, 注释, sheet名, 文件名}} 和 dict {变量名: {io地址, 注释, sheet名, 文件名}}
    """
    # IO表数据（用于IO地址匹配）
    io_table_by_address = {}
    io_table_by_name = {}
...
def comments_match(comment1, comment2, exact=True):
    """
    检查两个注释是否匹配
    exact=True: 精确匹配
    exact=False: 模糊匹配（去除常见后缀后比较）
    """
    if not comment1 or not comment2:
        return False
    if exact:
        return comment1.strip() == comment2.strip()
    # 模糊匹配
    return normalize_comment(comment1) == normalize_comment(comment2)
...
def detect_duplicates(io_by_address_all, io_by_name_all):
    ...
    返回: {
        ...
        'plc_var_duplicates': {var_name: [records]}      # PLC变量重复
    }
    """
    result = {
...
def check_source_io_match(
    ...
    if not db_records and io_by_name_all:
        # 从合并数据中筛选IO数据库的记录
        db_records = {}
        for name, records in io_by_name_all.items():
            db_records[name] = [
    ...
    for var in plc_vars:
        ...
        if comment:
            # 使用IO数据库记录进行变量匹配
            # 首先尝试用完整变量名查找（如 YTJ_QJ_SJ1.Sr_I）
        ...
        if comment_matches:
            # 找到IO名称匹配的记录，现在比较变量名
            # 比较方式1：cleaned_name == clean_var_name（基础名匹配，如 YTJ_QJ_SJ1 == YTJ_QJ_SJ1）
            # 比较方式2：plc_control_var == clean_var_name（完整名匹配，如 YTJ_QJ_SJ1.Sr_I == YTJ_QJ_SJ1.Sr_I）
            var_matches = []
        ...
        if io_addr and io_by_address_all and io_addr in io_by_address_all:
            ...
            if locations:
                ...
                if len(unique_locations) == 1:
                    io_addr_remark = f"IO地址匹配:{unique_locations[0]}"
                else:
...
def check_sr_io_mismatch(plc_vars, io_by_address):
    """
    检查Sr_I/Sr_O是否接反
...
def check_normal_cylinder_not(plc_vars, io_by_address, has_reverse_cylinder_col=False):
    """
    检查正常气缸（非反气缸）是否**缺少**NOT（正常气缸应该加NOT）
    返回: list of dict {气缸名称, io地址, 说明, 源码}
    """
    # 如果没有反气缸列，跳过检查
    if not has_reverse_cylinder_col:
...
def infer_prefix(var_name):
    """
    根据变量名推断模块前缀
    使用精确匹配避免子串误判（如ZLTJC中的TJ不应被识别为贴胶）
    """
    var_upper = var_name.upper()
...
def extract_device_base_name(comment):
    """
    从Excel注释中提取设备基础名（去掉回限/出限/报警等后缀）
    例如：'拐角贴胶缓存_拐角胶上切胶气缸回限' -> '拐角贴胶缓存_拐角胶上切胶气缸'
    """
    if not comment:
        return None


def explain_variable_name(var_name):
    """
    解释变量名的含义
    返回变量名的中文解释
    """
...
def clean_plc_source(plc_files, output_dir, timestamp):
    """
...
def save_sheet_analysis(sheet_analysis, output_dir):
    """
    保存每个Sheet的IO识别分析详情到文件，供后续核对和复盘
    """
