...
## 2026-09-12 追加：MCP 失败结果被永久缓存（一次 DNS 竞态 = 容器生命周期内工具不可用）

**问题**：1.0.10 部署（16:15）后 decrypt-file 全程不可用，agent 被迫走 base64
绕路导出 Excel。1.0.11（16:59）重启后"自愈"——纯运气：mcp 容器恰好先就绪。

**根因（两个缺陷叠加）**：
1. stack 重新发布时 agent 与 geesun-mcp 同批重启，agent 先就绪 → swarm DNS 里
   `geesun-mcp` 名尚未注册 → 首次连接 "Temporary failure in name resolution"；
2. `mcp.py` 旧代码 `get_mcp_tools` **无论成败都写 `_tools_cache`**——而它每轮
   chat 都被调用（chat.py:354）。失败（空工具列表）被缓存 = 一次 3 秒的 DNS
   竞态被放大成整个容器生命周期内该 server 永久不可用。

**修复（commit 见 git log）**：connect_one 退避重试 2 次（5s 超时 + 2s 退避，
最坏 ~12s/轮）；有失败就不写缓存，下一轮 chat 自动重连（成功 server 重连成本
仅一次 HTTP 握手）。spike：`tests/spikes/mcp_connect_resilience.py`（9 断言），
其中「失败 return 的行号必须早于缓存写入行号」是位置断言，防回归。

**教训**：
- **缓存失败结果 = 把瞬态故障固化成永久故障**。凡"每请求都走 + 结果进缓存"
